...
## Segmentation Integration (Cityscapes)

For semantic segmentation training with a frozen DINOv3 backbone and a linear decoder, precompute numpy TAM maps using `precompute_tam.py` in the project root:

```
python precompute_tam.py --root /path/to/cityscapes --split train --out_dir tam_maps_train
python precompute_tam.py --root /path/to/cityscapes --split val   --out_dir tam_maps_val
```

Each `.npy` file stores an array `(19, H_t, W_t)` of activation maps (missing classes -> zeros) ordered by Cityscapes trainId. These are later interpolated to the ViT patch grid and concatenated to DINOv3 patch features before the linear head.
